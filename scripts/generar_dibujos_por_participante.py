"""
generar_dibujos_por_participante.py

Idéntico a generar_dibujos_por_registro.py -- genera una imagen por
cada fila de la tabla `annotations`, mostrando exactamente lo que esa
persona dibujó (respetando el orden real de sus trazos, incluyendo lo
que borró). La única diferencia es el nombre de los archivos: en vez
del id de la anotación, usa el id del participante seguido del id de
la imagen (un participante anota varias imágenes, así que se necesitan
ambos para que ningún archivo se sobrescriba).

Requisitos (instalar una sola vez):
    pip install supabase pillow numpy requests python-dotenv

Uso:
    1. Usa el mismo ".env" que ya tienes para los otros scripts
       (SUPABASE_URL y SUPABASE_SERVICE_ROLE_KEY).
    2. Corre: python generar_dibujos_por_participante.py
    3. Revisa la carpeta "dibujos_por_participante/" -- un .png por
       cada anotación, más un "manifiesto.json".
"""
...
OUTPUT_DIR = "dibujos_por_participante"
...
def main():
    ...
    for fila in anotaciones:
        ...
        nombre_archivo = f"{fila['participant_id']}__{image_id}.png"
